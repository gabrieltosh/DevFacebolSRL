...
<div class="card">
    <div class="card-header">
        <div class="row">
        	<div class="col-sm-6">
        		<h2><strong>EMPRESAS REGISTRADAS</strong></h2>
        	</div>
        	<div class="col-sm-6">
        		<div class="pull-right">
                    <br>
        			<a href="{{route('empresas.create')}}" class="btn btn-info"><i class="zmdi zmdi-plus"></i> Nuevo Registro</a>
        		</div>
        	</div>
        </div>
	        ...
	        <tbody>
	            @foreach($empresas as $empresa)
	                    <tr>
                            ...
                            <td class="text-center">{{$empresa->imagen}}</td>
	                        <td class="text-center">{{$empresa->ciudad->nombre}}</td>
                            <td class="text-center">{{$empresa->activo}}</td> 
	                        <td  class="text-center"> 
	                        </td> 
	                    </tr>      	                   
	                 	@include('panel.empresas.show')
	                	@include('panel.empresas.delete')
	                	@include('panel.empresas.estado')
	            @endforeach       
	        </tbody>
	    </table>
	</div>
        ...
        formatters: {
            ...
            "commands": function(column, row) {
            	if(row.activo==1)
            	{
            		...
            		var edit="<a href=\"/panel/empresas/"+row.id+"/edit\" class=\"btn btn-icon command-edit waves-effect waves-circle\"><span style=\"color: #00bcd4;\" class=\"zmdi zmdi-edit\"></span></a> " ;
            ...
            "imagen": function(column,row){
            	return "<img src=\"{{asset('imagen')}}/empresas/" +row.imagen+"\" width=\"50px\" height=\"50px\" class=\"img-responsive img-thumbnail\" data-row-id=\"" + row.id + "\">";
            },
