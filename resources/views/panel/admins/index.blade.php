...
<div class="card">
    <div class="card-header">
        <div class="row">
        	<div class="col-sm-6">
        		<h2><strong>ADMINISTRADORES REGISTRADOS</strong></h2>
        	</div>
        	<div class="col-sm-6">
        		<div class="pull-right">
                    <br>
        			<a href="{{route('administradores.create')}}" class="btn btn-info"><i class="zmdi zmdi-plus"></i> Nuevo Registro</a>
        		</div>
        	</div>
        </div>
	        ...
	        <tbody>
	            @foreach($admins as $admin)
	                    <tr>
	                        ...
	                        <td class="text-center">{{ucwords($admin->apellido)}}</td>
	                        <td class="text-center">{{$admin->imagen}}</td>
	                        <td class="text-center">{{$admin->celular}}</td>
	                        <td class="text-center">{{$admin->activo}}</td>
	                        <td  class="text-center"> 
	                        </td> 
	                    </tr>         
	                @include('panel.admins.show')
	                @include('panel.admins.delete')
	                @include('panel.admins.estado')
	            @endforeach     
	        </tbody>
	    </table>
	</div>
        ...
        formatters: {
            ...
            "commands": function(column, row) {
            	if(row.activo==1)
            	{
            		...
            		var edit="<a href=\"/panel/administradores/"+row.id+"/edit\" class=\"btn btn-icon command-edit waves-effect waves-circle\"><span style=\"color: #00bcd4;\" class=\"zmdi zmdi-edit\"></span></a> " ;
