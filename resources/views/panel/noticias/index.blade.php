...
<div class="card">
    <div class="card-header">
        <div class="row">
        	<div class="col-sm-6">
        		<h2><strong>NOTICIAS REGISTRADAS</strong></h2>
        	</div>
        	<div class="col-sm-6">
        		<div class="pull-right">
        			<a href="{{route('noticias.create')}}" class="btn btn-info"><i class="zmdi zmdi-plus"></i> Nuevo Registro</a>
        		</div>
        	</div>
        </div>
	    ...
	    <table id="data-table-command" class="table table-striped table-vmiddle">
	        ...
	        <tbody>
	            @foreach($actividades as $actividad)
                        ...
                        @include('panel.noticias.delete')
	                	@include('panel.noticias.estado')
	            @endforeach       
	        </tbody>
	    </table>
	</div>
        ...
        formatters: {
            ...
            "commands": function(column, row) {
            	if(row.activo==1)
            	{
            		...
            		var edit="<a href=\"/panel/noticias/"+row.id+"/edit\" class=\"btn btn-icon command-edit waves-effect waves-circle\"><span style=\"color: #00bcd4;\" class=\"zmdi zmdi-edit\"></span></a> " ;
